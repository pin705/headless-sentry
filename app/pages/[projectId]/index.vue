...
const errorColumns = [
  { accessorKey: 'monitor', header: 'Dịch vụ',
    cell: ({ row }: any) => {
      ...
      return monitor
        ? h('span', { class: 'text-sm font-medium hover:underline cursor-pointer', onClick: () => navigateTo(`/monitoring/${monitor._id}`) }, monitor.name) // Link tới trang chi tiết
...
const alertColumns = [
  { accessorKey: 'name', header: 'Dịch vụ',
    cell: ({ row }: any) => h('span', { class: 'text-sm font-medium hover:underline cursor-pointer', onClick: () => navigateTo(`/monitoring/${row.original._id}`) }, row.original.name) // Link tới trang chi tiết
  },
